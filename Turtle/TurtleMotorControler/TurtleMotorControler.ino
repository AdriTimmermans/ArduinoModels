...
void receiveMasterMessage(int howMany)
{
  ...
  byte mWheelCode; // 0b00 = stop, 0b01 = forward full, 0b10 = backward full
  ...
  switch (messageTypeForMotor)
  {
    case informMotorOrder :
      ...
      Serial.print(oneMessage.motorId);
      mWheelCode = Wire.read();
      Serial.print(", wheel code  = ");
      Serial.println(mWheelCode);
     
      if (mWheelCode == 0)
      {
        oneMessage.motorDirection = 0;
      }
      else
      {
        (mWheelCode == 0b00000010)?oneMessage.motorDirection = 1:oneMessage.motorDirection = 2;
      }
